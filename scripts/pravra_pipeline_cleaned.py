# â”€â”€ Section 1: Data already extracted to local path â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
import os, json
...
# â”‚              r"E:/Pravara-River-Basin-Advanced-Morphometric-Hydrological-Analysis-main/Pravara-River-Basin-Advanced-Morphometric-Hydrological-Analysis-main/data/watershed_data/pravra3.shp"                        â”‚
...
# â”€â”€ Detect stream order column dynamically â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
for cand in ORDER_COL_CANDIDATES:
    if cand in gdf_so.columns:
        ORDER_COL = cand
        print(f"  âœ… Stream order column auto-detected: '{ORDER_COL}'")
        break
...
assert HILLSHADE.shape == DEM_ARR.shape, \
    f"Hillshade shape {HILLSHADE.shape} â‰  DEM shape {DEM_ARR.shape}. Recompute hillshade from the reprojected DEM."
...
# â”€â”€ CRS sanity check â€” all layers must be in the same UTM zone â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
ref_crs = gdf_sub.crs
for name, gdf in [('streams', gdf_streams), ('stream_order', gdf_so)]:
    if gdf.crs != ref_crs:
        print(f"  âš ï¸  CRS mismatch: {name} is {gdf.crs}, expected {ref_crs}")
...
# â”€â”€ Correct: clip stream network to each subbasin, then tag basin_id â”€â”€â”€â”€â”€â”€â”€â”€â”€
clipped_parts = []
...
if clipped_parts:
    ...
    print("  âš ï¸  Falling back to intersects join â€” check CRS alignment.")
...
def longest_flow_path(basin_geom, facc_arr=None, transform=None, res_m=None):
    ...
    bounding rectangle â€” a standard GIS proxy used when full D8 tracing is
...
for _, row in gdf_sub.iterrows():
    ...
    # â”€â”€ Correct Texture Ratio: use first-order streams only â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Hypsometric Curves â€” all sub-basins â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if 'HYPS' in globals() and HYPS:
    ...
    ax_h.set_title("Hypsometric Curves â€” Pravara Sub-basins", fontsize=13,
                 fontweight='bold')
    ...
    plt.close(fig_h)
    print("  âœ… Hypsometric curve plot saved.")


# â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
# â•‘  MAP FRAMEWORK â€” 70:30 layout Â· ESRI basemap Â· GIS scale bar          â•‘
# â•‘  DESIGN-01 Â· DESIGN-02 Â· DESIGN-03 Â· DESIGN-04 Â· DESIGN-05 Â· 07     â•‘
...
# â”€â”€ Basemap sources â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ DESIGN-05: Mask raster to basin boundary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Basin extent with padding â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ DESIGN-01: Standard GIS Unified Layout Figure Factory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def add_esri_basemap(ax, source=None, alpha=0.55):
    ...
    if source is None: source = cx.providers.Esri.WorldTopoMap
    try:
        cx.add_basemap(ax, crs=str(gdf_sub.crs), source=source,
...
#  PROFESSIONAL COMPASS ROSE â€” drawn entirely in matplotlib (no PNG needed)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# â”€â”€ DESIGN-02: Professional cartographic compass rose (classical 8-point) â”€â”€
def add_compass_rose_panel(ax_panel, cx_pos=0.08, cy_pos=0.15, r=0.06):
    """Draws a classical 8-point compass rose. Now positioned as a bottom-left overlay."""
    circle = plt.Circle((cx_pos, cy_pos), r*1.15, transform=ax_panel.transAxes,
                        facecolor='white', alpha=0.8, edgecolor='#333333', lw=0.6, zorder=30)
...
# â”€â”€ DESIGN-03: Professional GIS scale bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# â”€â”€ DESIGN-03: Modular GIS staggered scale bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def add_scale_bar_panel(ax_panel, utm_extent=None, y_top=0.32, x_left=0.03, bar_w=0.15):
    """Draws a professional GIS scale bar as a floating overlay."""
    if utm_extent is None: utm_extent = compute_utm_extent()
    dist_km = (utm_extent[1] - utm_extent[0]) / 1000.0
    bar_km = next((n for n in [5,10,20,50,100] if n >= dist_km*0.15), 10)
    ...
    ax_panel.add_patch(Rectangle((x_left-0.01, y_top-0.08), bar_w+0.02, 0.12, transform=ax_panel.transAxes,
                                 facecolor='white', alpha=0.8, edgecolor='none', zorder=20))
...
# â”€â”€ DESIGN-06: Classified legend in panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def add_classified_legend_panel(ax_panel, colours, labels,
                                  title='Legend', y_top=0.42, x_left=0.74):
    """Categorical legend in an inset box."""
    n = len(labels)
    box_w, box_h = 0.24, 0.05 + (n * 0.045)
    
    # Boxed furniture container
    ax_panel.add_patch(FancyBboxPatch((x_left, y_top-box_h), box_w, box_h, 
                                     boxstyle='round,pad=0.01', transform=ax_panel.transAxes,
                                     facecolor='white', alpha=0.92, edgecolor='#1a1a1a', lw=1.2, zorder=100))
...
def add_continuous_legend_panel(ax_panel, arr_masked, cmap_name, label,
                                  y_top=0.50, y_bot=0.05, log_scale=False):
    from matplotlib.colorbar import ColorbarBase
    from matplotlib.colors import Normalize, LogNorm
    # 'Legend' heading
    ax_panel.text(0.50, y_top + 0.022, 'Legend',
                  transform=ax_panel.transAxes, ha='center', va='bottom',
                  fontsize=9, fontweight='bold', color='#1a1a1a')
    # Colourbar â€” centred, tall
    cb_ax = ax_panel.inset_axes([0.32, y_bot, 0.20, y_top - y_bot])
    valid = arr_masked[~np.isnan(arr_masked)]
    vmin  = float(np.nanpercentile(valid, 2))  if len(valid) else 0
    vmax  = float(np.nanpercentile(valid, 98)) if len(valid) else 1
    if log_scale and vmin > 0:
        norm = LogNorm(vmin=max(vmin, 1e-6), vmax=vmax)
    else:
        norm = Normalize(vmin=vmin, vmax=vmax)
    cb = ColorbarBase(cb_ax, cmap=plt.get_cmap(cmap_name),
                      norm=norm, orientation='vertical')
    cb.set_label(label, fontsize=7.5, labelpad=4)
...
def finalize_map(fig, ax_map, ax_panel, filename, add_panel_elements=True,
                 source=BASEMAP_HILLSHADE, basemap_alpha=0.45):
    """Clamp to basin extent, apply 4-sided DÂ°M' grid, add panel elements, save."""
    ext = basin_extent_padded()
    ax_map.set_xlim(ext[0], ext[1])
    ax_map.set_ylim(ext[2], ext[3])
    apply_dms_grid(ax_map, ext)
    # Neatline â€” thick border around map
    for spine in ax_map.spines.values():
        spine.set_linewidth(1.5)
        spine.set_edgecolor('#1a1a1a')
    if add_panel_elements:
        add_compass_rose_panel(ax_panel)   # no x= y= â€” uses defaults
        add_scale_bar_panel(ax_panel, compute_utm_extent())
...
# Backward-compat shim so old code calling finalize_and_save still works
def finalize_and_save(fig, ax, utm_extent, filename, n_ticks=4):
    """Legacy shim updated for the new Unified Frame."""
    ax_panel = getattr(fig, '_panel_ax', None)
    if ax_panel is not None:
        finalize_map(fig, ax, ax_panel, filename)
...
# Backward-compat shim â€” base_axes for sections 14/15/16/18 (old-style choropleth)
# Returns (fig, ax_map, utm_extent) like the old code expected
def base_axes(title, figsize=(16, 10)):
    """70:30 shim: creates make_map_figure but returns (fig, ax_map, utm_extent)
    so that old code   fig, ax, utm_ext = base_axes(title)  still works.
    The panel is stored as fig._panel_ax for use by finalize_and_save_shim."""
    fig, ax_map, ax_panel = make_map_figure(title, figsize=figsize)
    fig._panel_ax = ax_panel   # stash for finalize_and_save
...
print("Map framework loaded â€” DESIGN-01 through DESIGN-07 active.")
...
def dd_to_dms(dd, is_lat=True):
    """Convert decimal degrees -> DÂ°M' format only (no seconds)."""
...
gdf_sub.boundary.plot(ax=ax_map, edgecolor='black', linewidth=1.5, zorder=10)
add_continuous_legend_panel(ax_panel, SLOPE_ARR_MASKED, 'YlOrRd', 'Slope (Â°)')
...
gdf_sub.boundary.plot(ax=ax_map, edgecolor='black', linewidth=1.5, zorder=10)
add_continuous_legend_panel(ax_panel, ASPECT_ARR_MASKED, 'hsv', 'Aspect (Â°)')
...
add_esri_basemap(ax_map2, source=BASEMAP_TOPO, alpha=0.65)
ax = ax_map2
...
# Smf proxy already assigned above â€” no rank used (BUG-08 uses 1-4 classify instead)
...
# â”€â”€ AF proxy: Relief Ratio used as surrogate for Asymmetry Factor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# True AF = 100Ã—(Ar/At); requires detailed sub-basin delineation.
...
# â”€â”€ T proxy: Drainage Density used as surrogate for Transverse Symmetry â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Vf proxy: Mean Slope used as surrogate for Valley Floor Width-Height Ratio â”€
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#  F. INDEX OF ACTIVE TECTONICS (IAT) â€” El Hamdouni et al. (2008)
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

# â”€â”€ Assign 1â€“4 class per index using published literature thresholds â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Smf: < 1.6 â†’ Class 1 (Very High); 1.6â€“3.0 â†’ 2; 3.0â€“5.0 â†’ 3; >5.0 â†’ 4
...
# AF proxy (Relief Ratio): >0.10 â†’ 1, 0.05â€“0.10 â†’ 2, 0.02â€“0.05 â†’ 3, <0.02 â†’ 4
...
# T proxy (Drainage Density km/kmÂ²): >3.0 â†’ 1, 2.0â€“3.0 â†’ 2, 1.0â€“2.0 â†’ 3, <1.0 â†’ 4
...
# Vf proxy (slope deg): >15Â° â†’ 1, 8â€“15Â° â†’ 2, 3â€“8Â° â†’ 3, <3Â° â†’ 4
...
# IAT = mean of individual 1â€“4 classes (El Hamdouni 2008)
...
def classify_iat(iat_score):
    if iat_score <= 1.5: return 'Class 1 â€” Very High'
    if iat_score <= 2.5: return 'Class 2 â€” High'
    if iat_score <= 3.5: return 'Class 3 â€” Moderate'
...
# Only major streams (order >= 3) to reduce visual noise â€” MAP-01 fix
...
def calculate_sti(stream_gdf, flow_acc_arr, slope_arr, dem_res, dem_transform=None, threshold=1e-6):
    ...
    for idx, row in stream_gdf.iterrows():
        ...
        # â”€â”€ Correct STI per Moore & Burch (1986) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # STI = (As/22.13)^0.6 Ã— (sin Î² / 0.0896)^1.3
        cell_area = dem_res * dem_res                   # mÂ²
...
def calculate_twi(dem_arr, dem_res):
    ...
    # â”€â”€ Compute TWI only where As > 0 and slope > 0 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ MAP-02: use robust 2-98 pct normalisation to eliminate black speckle â”€â”€
...
# Weighted FFPI â€” MAP-08: apply flat-area masking on each component
...
# â”€â”€ MAP-06/07/08: 70:30 layout for all hazard rasters with log-stretch â”€â”€â”€â”€â”€â”€â”€
...
add_continuous_legend_panel(ax_panel13b, SPI_LOG_MASKED, 'YlOrRd', 'logâ‚â‚€(SPI) [mÂ²]')
...
add_continuous_legend_panel(ax_panel13c, STI_LOG_MASKED, 'RdPu', 'logâ‚â‚€(STI)')
...
add_continuous_legend_panel(ax_panel13d, FFPI_MASKED, 'OrRd', 'FFPI (0â€“1)')
finalize_map(fig, ax_map13d, ax_panel13d, "13d_FFPI_map.png")

# â”€â”€ MAP-09: Composite flood hazard choropleth (70:30, quantile-based) â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ BUG-09: defensive guard if section reruns independently â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ LS-factor exponents (Moore et al., 1991) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# m=0.4, n=1.3 â†’ interrill/sheet erosion (standard default)
# m=0.6, n=1.3 â†’ rill-dominated / steep terrain (change if confirmed by field)
m_exp = 0.4   # âœ… standard Moore et al. (1991) â€” Pravara basin Deccan trap Vertisols
n_exp = 1.3
